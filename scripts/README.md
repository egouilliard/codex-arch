...
The `validate-dependencies` and `fix-dependencies` commands help maintain valid task dependencies:
...
Additionally, this folder contains utility scripts for analyzing imports in Python codebases:

### find_imports.py

This script finds all files that import a specific target file.

```bash
python scripts/find_imports.py <dependencies_file> <target_file> [--exact] [--output-dir <dir>]
```

Parameters:
- `dependencies_file`: Path to the dependency JSON file generated by Codex-Arch
- `target_file`: The file to find importers for
- `--exact`: Optional flag to match exact file path instead of just filename
- `--output-dir`, `-o`: Optional output directory where dependency files are located

Example:
```bash
# Specify a dependency file directly
python scripts/find_imports.py output/complete_dependencies.json pydantic_ai_coder.py

# Use the output-dir parameter to simplify path handling
python scripts/find_imports.py complete_dependencies.json pydantic_ai_coder.py --output-dir output
```

### find_all_importers.py

This script finds all files that import any file matching a pattern.

```bash
python scripts/find_all_importers.py <dependencies_file> <target_pattern> [--output-dir <dir>]
```

Parameters:
- `dependencies_file`: Path to the dependency JSON file generated by Codex-Arch
- `target_pattern`: Regex pattern to match against file paths
- `--output-dir`, `-o`: Optional output directory where dependency files are located

Example:
```bash
# Specify a dependency file directly
python scripts/find_all_importers.py output/complete_dependencies.json "models.*\.py"

# Use the output-dir parameter to simplify path handling
python scripts/find_all_importers.py complete_dependencies.json "models.*\.py" --output-dir output
```
